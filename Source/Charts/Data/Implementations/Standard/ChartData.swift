...
open class ChartData: NSObject
{
    ...
    {
        _dataSets.forEach {
            $0.calcMinMaxY(fromX: fromX, toX: toX)
        }
    ...
    internal func getDataSetIndexByLabel(_ label: String, ignorecase: Bool) -> Int
    {
        if ignorecase
        {
            return dataSets.firstIndex {
                guard let l = $0.label else {
                    return false
                }
                return label.caseInsensitiveCompare(l) == .orderedSame
            } ?? -1
        }
        else
        {
    ...
    @objc @discardableResult open func removeDataSet(_ dataSet: IChartDataSet!) -> Bool
    {
        ...
        guard let i = dataSets.firstIndex(where: { $0 === dataSet }) else {
            return false
        }
        removeDataSetByIndex(i)
    ...
    @objc open func getDataSetForEntry(_ e: ChartDataEntry!) -> IChartDataSet?
    {
        ...
        return dataSets.first {
            e === $0.entryForXValue(e.x, closestToY: e.y)
        }
    }
    ...
    @objc open var highlightEnabled: Bool
    {
        get
        {
            return dataSets.allSatisfy {
                $0.isHighlightEnabled
            }
        }
        set
        {
